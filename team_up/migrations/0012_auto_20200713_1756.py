# Generated by Django 3.0.8 on 2020-07-13 17:56

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        ('accounts', '0009_auto_20200623_2033'),
        ('team_up', '0011_auto_20200712_1848'),
    ]

    operations = [
        migrations.AlterField(
            model_name='recruited_teammates',
            name='teammates',
            field=models.ForeignKey(default=1, on_delete=django.db.models.deletion.CASCADE, related_name='teammates', to='accounts.Extendeduser'),
            preserve_default=False,
        ),
        migrations.AlterField(
            model_name='teams',
            name='logged_in_user',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='accounts.Extendeduser'),
        ),
    ]
