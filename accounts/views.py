...
def signup(request):
	if request.method == 'POST':
		...
		if(gender_validation and email_validation):
			creds_validation = user.validate(request.POST.get('username'),
			request.POST.get('first_name'),
			request.POST.get('gender'), request.POST.get('email'),
			request.POST.get('city'), 
			request.POST.get('country'), request.POST.get('password1'),
			...
			return render(request, 'accounts/signup.html', 
			{'error': 'Check if your email'+
				' is in correct format.'})
		elif gender_validation == False:
			return render(request, 'accounts/signup.html', 
			{'error': 'Check if you have' + ' choosen the Gender correctly.'})
...
def edit_profile(request):
	...
	if request.method == 'POST':
		if extendeduser.email_validate(request.POST.get('email')):
			extendeduser.user_details(request.POST.get('first_name'), 
			request.POST.get('last_name'), request.POST.get('gender'),
			request.POST.get('email'), request.POST.get('city'),
			request.POST.get('country'), request.POST.get('phone'), 
			request.POST.get('about_me'), request.POST.get('portfolio'),
			request.POST.get('linkedin'), request.POST.get('facebook'),
			request.POST.get('twitter'), request.POST.get('other_url'),
			request.user)
			return redirect('user_profile')
		else:
			return render(request, 'user_profile/update.html', 
			{'error': 'Check if your email'+ ' is in correct format.',
			'data':Extendeduser.objects.filter(user = request.user)})
	else:
